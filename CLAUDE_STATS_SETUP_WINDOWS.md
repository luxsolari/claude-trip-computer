# Claude Code Stats Tracking Setup for Windows

This guide provides step-by-step instructions to replicate the session statistics tracking functionality (custom slash command and status line) for Claude Code on Windows machines.
...
### For WSL (Recommended)
- Windows Subsystem for Linux (WSL) with Ubuntu or similar
- `jq` - JSON processor
- `bc` - Command-line calculator
- `bash` - Shell (included in WSL)

### For Native Windows (Git Bash/MSYS2)
- Git Bash or MSYS2 installed
- `jq` for Windows
- `bc` for Windows (included in Git Bash/MSYS2)
...
# If not installed, download jq for Windows from https://stedolan.github.io/jq/download/
# Place jq-win64.exe in C:\Program Files\Git\usr\bin\ and rename to jq.exe

# bc is included in Git Bash - verify:
bc --version
```
...
```bash
# For WSL or Git Bash, create the directory structure
...
Edit or create `~/.claude/settings.json` to add the status line configuration:

```bash
# Create or edit the settings file
# If the file doesn't exist, create it with the statusLine config:
...
# If the file already exists, use jq to merge the statusLine config:
# jq '.statusLine = {"type": "command", "command": "~/.claude/hooks/brief-stats.sh"}' ~/.claude/settings.json > ~/.claude/settings.json.tmp && mv ~/.claude/settings.json.tmp ~/.claude/settings.json
```
...
# Place in Git Bash bin directory (e.g., C:\Program Files\Git\usr\bin\)
```
...
# Git Bash: bc is included by default
```
...
- Example: `/Users/user/Code/my_project` → `~/.claude/projects/-Users-user-Code-my-project/`

### Issue: Wrong home directory in WSL
```bash
# If using WSL, ensure you're using the Linux home directory (~)
# Not the Windows home directory (/mnt/c/Users/...)
echo $HOME  # Should show /home/username

# If scripts reference wrong paths, edit them to use the correct home
```

### Issue: Path format differences (WSL vs Windows)
- WSL uses Unix-style paths: `/home/user/.claude/`
- Git Bash uses Windows-style paths: `/c/Users/user/.claude/`
- Ensure the scripts use paths compatible with your environment

## Path Adjustments for Windows Native (Git Bash)

If using Git Bash on Windows (not WSL), you may need to adjust paths:

```bash
# Windows Git Bash uses:
# ~/.claude  →  /c/Users/YourUsername/.claude/
# pwd returns: /c/Users/YourUsername/Code/project

# The scripts should work as-is, but verify:
echo $HOME  # Should show /c/Users/YourUsername
ls ~/.claude/  # Should show hooks and commands directories
```
...
To switch from API to subscription (WSL/Git Bash):
```bash
unset ANTHROPIC_API_KEY
# Then use /login in Claude Code to authenticate with your subscription
```

To switch from subscription to API (WSL/Git Bash):
```bash
...
```cmd
# To unset (switch to subscription):
set ANTHROPIC_API_KEY=

# To set (switch to API):
...
5. Verify paths are correct for your environment (WSL vs Git Bash)
