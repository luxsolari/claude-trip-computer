# Claude Code Stats Tracking Setup for Linux

This guide provides step-by-step instructions to set up session statistics tracking functionality (custom slash command and status line) for Claude Code on Linux systems.
...
# RHEL/CentOS/Fedora:
sudo dnf install -y bc  # or: sudo yum install -y bc
...
- Example: `/home/user/Code/my_project` → `~/.claude/projects/-home-user-Code-my-project/`
...
### Issue: Unicode characters not displaying (emojis)
- Ensure your terminal supports UTF-8 encoding
- Check your locale: `locale | grep UTF`
- If needed, set UTF-8 locale:
  ```bash
  export LC_ALL=en_US.UTF-8
  export LANG=en_US.UTF-8
  ```

### Issue: `read: -t: invalid option` (on older bash versions)
```bash
# Check bash version
bash --version

# If bash version is < 4.0, the timeout feature may not work
# Upgrade bash or modify the scripts to remove the timeout
```

## Distribution-Specific Notes

### Ubuntu/Debian
- All tools available via `apt-get`
- Default bash version supports all features
- UTF-8 locale typically enabled by default

### RHEL/CentOS/Fedora
- Use `dnf` for version 8+ or `yum` for version 7
- May need to enable EPEL repository for some packages
- SELinux may require additional configuration (typically no issues)

### Arch Linux
- All tools available via `pacman`
- Rolling release ensures latest versions
- UTF-8 locale enabled by default

### openSUSE
- Use `zypper` package manager
- All tools readily available
- YaST can be used for graphical configuration
...
- Scripts are compatible with bash 4.0+ (available on all modern Linux distributions)
...
5. Review system logs if scripts fail silently
6. Ensure terminal supports UTF-8 for emoji display
