...
# Cost Estimate Safety Margin (conservative buffer to avoid underestimation)
# 1.00 = exact estimate, 1.05 = 5% buffer, 1.10 = 10% buffer
...
# Claude Code Session Stats - Version 0.6.6
...
echo "💬 $USER_MESSAGES msgs | 🔧 $TOOL_CALLS tools | 🎯 ${FORMATTED_TOKENS} tok | ⚡ ${CACHE_EFFICIENCY}% eff | $BILLING_ICON ~\$$FORMATTED_COST (\$$COST_PER_MESSAGE/msg) | 📊 /trip-computer"

exit 0
SCRIPT_EOF

chmod +x ~/.claude/hooks/brief-stats.sh
echo "✓ Installed brief-stats.sh (status line)"
...
SAFETY_MARGIN="${SAFETY_MARGIN:-1.05}"
...
if [ "$BILLING_MODE" = "Sub" ]; then
  ...
  USAGE_HEADER="📊 SESSION USAGE ESTIMATE (API Billing)"
  USAGE_NOTE="  💡 Run /cost separately to see official billing data and compare accuracy."
